...
bool CDiscController::Update()
{
	...
	else
	{
		...
		if (m_iIterations == 0) {
			...
			outputFile = ofstream("results.csv");
			outputFile << "Iteration number" << "," << "Average number of mines swept" << "," << "Maximum number of mines swept" << endl;
			outputFile << m_iIterations << "," << (sum / float(m_vecSweepers.size())) << "," << maxMines << endl;
		}
		else {
			outputFile = ofstream("results.csv", ios_base::app);
			outputFile << m_iIterations << "," << (sum / float(m_vecSweepers.size())) << "," << maxMines << endl;
		}
		outputFile.close();
